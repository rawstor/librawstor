...
std::string join(const rawstd::URI& parent, const std::string& child) {
...
void parse_uri(
    ...
    /*
     * Userinfo lives in the authority only: an '@' at or past the first
     * '/' is path data (e.g. an "<uuid>@<snap_id>" object reference).
     */
    ...
    if (at_delim != uri.npos && at_delim < authority_end) {
        at_delim += 1;
...
std::vector<rawstd::URI> URI::uriv(const char* uris) {
...
std::string URI::uris(const std::vector<rawstd::URI>& uriv) {
    std::ostringstream oss;
    bool comma = false;
