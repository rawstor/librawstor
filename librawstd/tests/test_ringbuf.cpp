...
TEST(RingBufTest, move_constructor) {
    rawstd::RingBuf<int> buf1(3);

    std::unique_ptr<int> i1 = std::make_unique<int>(1);
    buf1.push(std::move(i1));
    EXPECT_EQ(buf1.size(), (size_t)1);

    rawstd::RingBuf<int> buf2(std::move(buf1));
