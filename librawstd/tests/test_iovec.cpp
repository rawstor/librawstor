...
TEST_F(IOVecTest, discard_front_unaligned) {
    ...
    size_t size = rawstd_iovec_discard_front(&iov_at, &niov_at, 12);
...
TEST_F(IOVecTest, discard_front_aligned) {
    ...
    size_t size = rawstd_iovec_discard_front(&iov_at, &niov_at, 10);
...
TEST_F(IOVecTest, discard_front_all) {
    ...
    size_t size = rawstd_iovec_discard_front(&iov_at, &niov_at, 30);
...
TEST_F(IOVecTest, discard_front_overflow) {
    ...
    size_t size = rawstd_iovec_discard_front(&iov_at, &niov_at, 35);
...
TEST_F(IOVecTest, discard_back_unaligned) {
    ...
    size_t size = rawstd_iovec_discard_back(&iov_at, &niov_at, 12);
...
TEST_F(IOVecTest, discard_back_aligned) {
    ...
    size_t size = rawstd_iovec_discard_back(&iov_at, &niov_at, 10);
...
TEST_F(IOVecTest, discard_back_all) {
    ...
    size_t size = rawstd_iovec_discard_back(&iov_at, &niov_at, 30);
...
TEST_F(IOVecTest, discard_back_overflow) {
    ...
    size_t size = rawstd_iovec_discard_back(&iov_at, &niov_at, 35);
...
TEST_F(IOVecTest, from_buf_unaligned) {
    ...
    size_t size = rawstd_iovec_from_buf(_iov, _niov, 0, buf, sizeof(buf) - 1);
...
TEST_F(IOVecTest, to_buf_unaligned) {
    char buf[12];
    size_t size = rawstd_iovec_to_buf(_iov, _niov, 0, buf, sizeof(buf));

    EXPECT_EQ(size, (size_t)12);
    EXPECT_EQ(strncmp(buf, "123456789012", size), 0);

    size = rawstd_iovec_to_buf(_iov, _niov, 3, buf, sizeof(buf));
...
TEST_F(IOVecTest, to_buf_aligned) {
    ...
    size = rawstd_iovec_to_buf(_iov, _niov, 10, buf, sizeof(buf));
...
TEST_F(IOVecTest, to_buf_all) {
    ...
    size = rawstd_iovec_to_buf(_iov, _niov, 3, buf, sizeof(buf) - 3);
...
TEST_F(IOVecTest, to_iovec) {
    ...
    size_t size =
        rawstd_iovec_to_iovec(_iov, _niov, 3, iov_dst.data(), iov_dst.size());
    ...
    size =
        rawstd_iovec_to_iovec(_iov, _niov, 20, iov_dst.data(), iov_dst.size());
...
TEST_F(IOVecTest, size) {
    EXPECT_EQ(rawstd_iovec_size(_iov, _niov), (size_t)30);
}
