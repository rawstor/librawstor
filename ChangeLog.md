...
- `--readonly` for `rawstor-vhost` and `rawstor-vduse`: advertises `VIRTIO_BLK_F_RO` and opens the target `RAWSTOR_READONLY`, which is also how a snapshot is exported.
