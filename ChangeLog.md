...
- `Connection::_with_retry()` now waits out an exponential backoff with jitter between I/O retry attempts instead of retrying immediately, using `rawio::Queue::timeout()`; tunable via the new `RAWSTOR_OPTS_IO_RETRY_BACKOFF_BASE`/`_MAX`/`_JITTER` environment variables (see README.md's "Environment Variables" section).
