...
### Changed
- `rawstor_target_open()` gained an `int flags` parameter after `target` (`0`, or `RAWSTOR_READONLY`): a read-only open needs no mirror write quorum (any one reachable member is enough; no mirror state is recorded, no read-repair or resync runs) and every write to the handle fails with `-EROFS`. Breaking C API change; existing callers pass `0`. `pyrawstor` gains `Target.open(flags=0)` (returning a new `Object` with `pread()`/`pwrite()`/`close()`) and `rawstor.READONLY`.
