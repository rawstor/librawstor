...
### Added
- Native CoW snapshots (`zfs://` only; `-ENOTSUP` elsewhere): `rawstor snapshot TARGET` (`rawstor_target_create_snapshot()`, unified with the plain-target native CoW path via a client-generated `snapshot_id` UUID, like every other object id) creates one; `rawstor remove TARGET/SNAPSHOT_ID` (`rawstor_target_remove()`, a target string that carries a bound snapshot version) removes it. Plus the new `rawstor_target_snapshot_id()` (`<rawstor/target.h>`).
