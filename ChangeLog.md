...
- `rawstor snapshot TARGET` / `rawstor snap-remove TARGET -s ID` (`rawstor_target_snapshot_create()`/`rawstor_target_snapshot_remove()`, unified with the plain-target native CoW path via `snap_id`'s input value -- 0 asks the object's MDS to assign one): MDS-orchestrated native CoW snapshots of an `mds://` object's chunks. `zfs://` chunk members only in this version; `file://`/classic LVM members answer `-ENOTSUP` (no fallback copies made behind the caller's back).
