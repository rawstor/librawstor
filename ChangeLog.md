...
- `rawstor-ost.service` exposes `--queue-size`/`--max-pending-writes` as the `QUEUE_SIZE`/`MAX_PENDING_WRITES` environment variables, overridable in `/etc/rawstor-ost.conf` like the rest of its tuning knobs.
