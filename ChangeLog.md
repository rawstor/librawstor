...
### Added
- Native CoW snapshots (`zfs://` only; `-ENOTSUP` elsewhere): `rawstor snapshot TARGET` / `rawstor snap-remove TARGET SNAP_ID`, and the underlying `rawstor_target_create_snapshot()`/`rawstor_target_snapshot_remove()`/`rawstor_target_snap_id()` API (`<rawstor/target.h>`).
...
- `rawstor_target_create()`/`rawstor_location_create()` reject `-EINVAL` for width `0`, or for a target/location string naming more than one URI whose width doesn't match that count exactly; a lone URI on a backend capable of its own internal redundancy (`ost://`) accepts any nonzero width as the caller's own chosen redundancy for that one copy, while `file://`, an LV, or a zvol -- exactly one physical copy each -- only accept `1`.
