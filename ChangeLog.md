...
### Added
- Native CoW snapshots (`zfs://` only; `-ENOTSUP` elsewhere): `rawstor snapshot TARGET [-u UUID]` (`rawstor_target_create_snapshot()`, `pyrawstor`'s `Target.create_snapshot()`) creates one, picking the version id one of three ways -- verbatim from an already-bound `TARGET`, a caller-chosen id via `-u`/`uuid=`, or a freshly generated one otherwise (combining an already-bound `TARGET` with `-u`/`uuid=` is rejected with `-EINVAL`) -- and returns the resulting `TARGET/SNAPSHOT_ID` target string (not just the bare id), ready to pass straight to `rawstor remove`/`rawstor_target_remove()` (a target string that carries a bound snapshot version) to remove it. `rawstor create`/`rawstor_target_create()` is now only ever for a fresh object -- passing it a `TARGET` that already carries a bound snapshot version fails with `-EINVAL` instead of taking a snapshot; `rawstor snapshot`/`rawstor_target_create_snapshot()` above is the only way to do that. A snapshot is opened with `rawstor_target_open(..., RAWSTOR_READONLY, ...)` -- the only way it can be opened. Plus the new `rawstor_target_snapshot_id()` (`<rawstor/target.h>`).
...
- `rawstor create` gained `--chunk-size`/`--failure-domain`/`--stripe-width` (an `mds://` object's chunk size, placement failure domain, and striping policy; ignored for a plain object).
...
- `rawstor snapshot TARGET [-u UUID]` (`rawstor_target_create_snapshot()`, `pyrawstor`'s `Target.create_snapshot()`) takes a native CoW snapshot of an `mds://` object, MDS-orchestrated across every one of its own chunks, and returns the resulting `TARGET/SNAPSHOT_ID` target string (not just the bare id); the version id is picked one of three ways -- verbatim from an already-bound `TARGET`, a caller-chosen id via `-u`/`uuid=`, or a freshly generated one otherwise (combining an already-bound `TARGET` with `-u`/`uuid=` is rejected with `-EINVAL`). `rawstor remove TARGET/SNAPSHOT_ID` (`rawstor_target_remove()`, a target string that carries a bound snapshot version) removes it. `rawstor create`/`rawstor_target_create()` is now only ever for a fresh object -- passing it a `TARGET` that already carries a bound snapshot version fails with `-EINVAL` instead of taking a snapshot; `rawstor snapshot`/`rawstor_target_create_snapshot()` above is the only way to do that. `zfs://` chunk members only in this version; `file://`/classic LVM members answer `-ENOTSUP` (no fallback copies made behind the caller's back). A snapshot is opened with `rawstor_target_open(..., RAWSTOR_READONLY, ...)` -- the only way it can be opened. Plus the new `rawstor_target_snapshot_id()` (`<rawstor/target.h>`).
...
### Changed
- A target string's internal chunk offset/snapshot version are now path segments instead of filename suffixes: `<uuid>/<snapshot_id>` (a plain target's own bound snapshot -- no offset segment, implied 0) or `<uuid>/<offset>/<snapshot_id>` (one chunk of a larger `mds://` object, offset always explicit) instead of `<uuid>:<offset>@<snapshot_id>`; `file://`'s own on-disk layout changed to match (`<uuid>/<offset>/data`, or `.../<snapshot_id>/data` for a bound snapshot), replacing the flat `<uuid>[:<offset>]` file it used to keep. Neither form was ever meant for manual entry (docs/locations_and_targets.md).
- `rawstor_target_remove()` now also removes a target's own bound snapshot, if its target string names one, the same way `rawstor_target_open()` already opened one. The OST wire protocol's own `RAWSTOR_CMD_RELEASE` merged what used to be a separate `RAWSTOR_CMD_SNAP_REMOVE` command the same way (nil `snapshot_id` for the live version); `RAWSTOR_CMD_SNAP_REMOVE` is retired.
- `rawstor_target_create()` now also takes a native CoW snapshot of the live version, ignoring `spec` (may be NULL), when its target string carries a bound snapshot version -- the same way `rawstor_target_remove()` above already dispatches removal; `rawstor_target_snapshot_id()` reads the bound version back off a target string built this way. `rawstor_target_remove_snapshot()` is gone -- `rawstor_target_remove()` above handles it instead.
- `rawstor_target_open()` gained an `int flags` parameter after `target` (`0`, or `RAWSTOR_READONLY`): a read-only open needs no mirror write quorum (any one reachable member is enough; no mirror state is recorded, no read-repair or resync runs) and every write to the handle fails with `-EROFS`. Breaking C API change; existing callers pass `0`.
