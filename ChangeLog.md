...
### Added
- Native CoW snapshots (`zfs://` only; `-ENOTSUP` elsewhere): `rawstor snapshot TARGET` / `rawstor snap-remove TARGET SNAPSHOT_ID`, and the underlying `rawstor_target_create_snapshot()`/`rawstor_target_remove_snapshot()`/`rawstor_target_snapshot_id()` API (`<rawstor/target.h>`).
