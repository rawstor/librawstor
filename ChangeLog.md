...
### Fixed
- On macOS, `librawio`'s `poll()` backend never suppressed `SIGPIPE`: writing to a socket whose peer had already closed its end could kill the process instead of failing the write with `EPIPE`.
