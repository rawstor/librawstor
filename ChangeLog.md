...
### Added
- Native CoW snapshots (`zfs://` only; `-ENOTSUP` elsewhere): `rawstor snapshot TARGET [-u UUID]` (`rawstor_target_create_snapshot()`, `pyrawstor`'s `Target.create_snapshot()`) creates one, picking the version id one of three ways -- verbatim from an already-bound `TARGET`, a caller-chosen id via `-u`/`uuid=`, or a freshly generated one otherwise (combining an already-bound `TARGET` with `-u`/`uuid=` is rejected with `-EINVAL`); `rawstor remove TARGET/SNAPSHOT_ID` (`rawstor_target_remove()`, a target string that carries a bound snapshot version) removes it. Plus the new `rawstor_target_snapshot_id()` (`<rawstor/target.h>`).
