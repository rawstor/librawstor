...
- `rawstor snapshot TARGET` / `rawstor snap-remove TARGET -s ID` (`rawstor_target_snapshot_create()`/`rawstor_target_snapshot_remove()`, unified with the plain-target native CoW path via a client-generated `snap_id` UUID, like every other object id): MDS-orchestrated native CoW snapshots of an `mds://` object's chunks. `zfs://` chunk members only in this version; `file://`/classic LVM members answer `-ENOTSUP` (no fallback copies made behind the caller's back).
...
### Changed
- A target string's internal chunk offset/snapshot version are now path segments (`<uuid>/<offset>/<snap_id>`) instead of filename suffixes (`<uuid>:<offset>@<snap_id>`); `file://`'s own on-disk layout changed to match (`<uuid>/<offset>/data`, or `.../<snap_id>/data` for a bound snapshot), replacing the flat `<uuid>[:<offset>]` file it used to keep. Neither form was ever meant for manual entry (docs/locations_and_targets.md).
