...
### Added
- Native CoW snapshots (`zfs://` only; `-ENOTSUP` elsewhere): `rawstor snapshot TARGET [-u UUID]` (`rawstor_target_create_snapshot()`, `pyrawstor`'s `Target.create_snapshot()`) creates one, picking the version id one of three ways -- verbatim from an already-bound `TARGET`, a caller-chosen id via `-u`/`uuid=`, or a freshly generated one otherwise (combining an already-bound `TARGET` with `-u`/`uuid=` is rejected with `-EINVAL`) -- and returns the resulting `TARGET/SNAPSHOT_ID` target string (not just the bare id), ready to pass straight to `rawstor remove`/`rawstor_target_remove()` (a target string that carries a bound snapshot version) to remove it. Plus the new `rawstor_target_snapshot_id()` (`<rawstor/target.h>`).
