...
- `Connection::_with_retry()` now waits out an exponential backoff with jitter (tunable via the new `RAWSTOR_OPTS_IO_RETRY_BACKOFF_BASE`/`_MAX`/`_JITTER`) instead of retrying immediately, and distinguishes a rejection from a live backend (retried against the same session, bounded by `RAWSTOR_OPTS_IO_ATTEMPTS`, skipped entirely for a rejection retrying can never fix, e.g. `ENOENT`) from a broken connection (reconnects and retries instead, bounded by the new `RAWSTOR_OPTS_IO_WIRE_RETRY_ATTEMPTS` -- `rawstor-vhost`/`rawstor-vduse` raise this to effectively unlimited by default, matching how a QEMU vhost-user chardev's own `reconnect=N` never gives up either). See README.md's "Environment Variables" section.
