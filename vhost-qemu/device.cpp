...
rawstd::DetachedTask pwritev_task(std::unique_ptr<Request> req) {
    // This device doesn't advertise VIRTIO_BLK_F_CONFIG_WCE on this branch
    // (that toggle isn't backported yet), so the guest assumes write-through
    // semantics and will never issue an explicit FLUSH for these writes --
    // sync=true is required for correctness here, not just a default.
...
rawstd::DetachedTask write_zeroes_task(std::unique_ptr<Request> req) {
    // Same reasoning as pwritev_task() above -- this device doesn't
    // advertise VIRTIO_BLK_F_CONFIG_WCE on this branch, so the guest
    // assumes write-through semantics and will never issue an explicit
    // FLUSH for WRITE_ZEROES either.
    int error = 0;
    try {
        std::vector<virtio_blk_discard_write_zeroes> segs =
            parse_dwz_segments(*req);
        for (const virtio_blk_discard_write_zeroes& seg : segs) {
            ...
            co_await co_object_write_zeroes(
                req->device().object(), size, offset, unmap, /*sync=*/true
            );
        }
    } catch (const std::system_error& e) {
        error = e.code().value();
