...
// rawio_read2()/rawio_write2() -- both share the same collapsed ssize_t
...
co_write(RawIOQueue* queue, int fd, const void* buf, size_t size) {
    rawstd::CallbackAwaitable<size_t> awaiter;
    int res = rawio_write2(queue, fd, buf, size, rawio_trampoline, &awaiter);
    if (res < 0) {
        RAWSTD_THROW_SYSTEM_ERROR(-res);
    }
    co_return co_await awaiter;
...
// rawio_read2()'s own collapsed ssize_t result callback shape, matching
