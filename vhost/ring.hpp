...
class Ring final {
    ...
    public:
        ...
        inline void set_num(unsigned int num) noexcept {
            _num = num;
        }

        inline unsigned int num() const noexcept {
            return _num;
        }
        ...
        inline uint16_t avail_flags() const noexcept {
            return _avail->flags;
        }

        inline uint16_t avail_idx() const noexcept {
            return _avail->idx;
        }
        ...
        inline uint16_t used_idx() const noexcept {
            return _used->idx;
        }

        inline void set_used_idx(uint16_t idx) noexcept {
            _used->idx = idx;
        }
