...
void read(RawIOQueue *queue, int fd, std::unique_ptr<TaskMessage> t) {
    int res = rawio_recvmsg(
        queue, fd, t->msg(), t->flags(),
        t->callback, t.get());
    if (res) {
        RAWSTD_THROW_SYSTEM_ERROR(-res);
    }
 ...
 */
std::unique_ptr<TaskWriteMsg> set_vring_enable(
    const std::shared_ptr<DeviceOp> &op)
{
    const VhostUserPayload &payload = op->payload();
 ...
 */
std::unique_ptr<TaskWriteMsg> get_vring_base(
    const std::shared_ptr<DeviceOp> &op)
{
 ...
 */
std::unique_ptr<TaskWriteMsg> set_config(
    const std::shared_ptr<DeviceOp> &op)
{
    const VhostUserHeader &header = op->header();
    const VhostUserPayload &payload = op->payload();
    ...
    try {
        ...
        rawstd_error("%s\n", e.what());
        return need_reply ?
            std::make_unique<TaskWriteU64>(op, 1) : nullptr;
    }
 ...
 */
std::unique_ptr<TaskWriteMsg> rem_mem_reg(
    const std::shared_ptr<DeviceOp> &op)
{
    const VhostUserHeader &header = op->header();
    VhostUserPayload &payload = op->payload();
 ...
 * acknowledge it.
 */
std::unique_ptr<TaskWriteMsg> reset_owner(
    const std::shared_ptr<DeviceOp> &)
{
    return nullptr;
}
    ...
    _postcopy_listening(false)
{
    ...
    try {
        ...
        _config.max_write_zeroes_sectors = 0; // VIRTIO_BLK_F_WRITE_ZEROES (unsupported)
        _config.max_write_zeroes_seg = 0;
        _config.write_zeroes_may_unmap = 0;

        _config.max_secure_erase_sectors = 0; // VIRTIO_BLK_F_SECURE_ERASE (unsupported)
...
void Device::set_features(uint64_t features) {
    if (!(features & (1ull << VIRTIO_F_VERSION_1))) {
        throw std::runtime_error(
            "virtio legacy devices are not supported");
    }
