...
static void command_snapshot_usage(void) {
    fprintf(
        ...
        "(docs/mds.md,\n"
        "\"Snapshots (stage 2)\"): the resulting TARGET/SNAPSHOT_ID is "
        "printed to\n"
        "stdout on success (status messages go to stderr). Not supported "
        "on an\n"
        "object with file:// or classic-LVM chunk members (-ENOTSUP, no "
        "fallback\n"
        "copies).\n"
        "\n"
        "  TARGET                An mds://host:port/<id> target, or one "
        "already\n"
