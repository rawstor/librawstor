...
| F10 | Chunk copy missing on one OST (disk lost, OST reprovisioned) | ENOENT when opening the copy | Treat as stale: recreate that slot (ALLOCATE) | Full online resync (F7) |
...
- **`cli/`** — `rawstor show -v` labels each mirror `mirror[N]` (its position in target's own comma-separated order) and prints its own state; `rawstor resolve TARGET --winner=N[,N...]` declares one or more mirrors jointly authoritative after split brain (F9), writing them all the same new dominant `sync_id` via `rawstor_target_set_sync_state()` so every mirror NOT listed gets a full resync on the next open. Still missing: `rawstor-cli force-open` / an opts flag (below-quorum start, explicit manual approval).
