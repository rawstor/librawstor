...
| F3 | All mirrors failed | all connections dead | At open: refused with `ENOTCONN` (degenerate case of F4's below-quorum refusal). On an already-open object: `EIO` to the caller on the next write, object unavailable. Periodic reconnect attempts | — (no copy available) |
