...
- **Roadmap — witness.** A metadata-only member (stores the consistency tuple, no data) restores auto-start for 2 data mirrors with one OST down. Designed in `rawstor_docs/Mds.md` (*Witness*, stage 3): it speaks the existing `SPEC`/`SET_STATE` subset (any OST or the MDS can host it), is written synchronously only at open / clean close / resync completion — never on the failure hot path — and its vote counts only when its record is provably consistent (`CLEAN` match, or a landed degrade update naming a single survivor); otherwise it abstains and these data-only quorum rules apply unchanged.
