...
ssize_t object_close(rawio::Queue& queue, RawstorObject* object) {
    return rawstor::tests::sync_run(&queue, [&](auto cb, void* data) {
        return rawstor_object_close2(object, cb, data);
    });
}

// rawstor_object_pread2()'s own callback shape (size_t result, int error)
// doesn't fit rawstor::tests::sync_run()'s (ssize_t result) convention --
// shared by every other rawstor_target_*()/rawstor_object_close2() call
...
ssize_t object_pread(
    ...
    int res = rawstor_object_pread2(object, buf, size, offset, cb, &r);
    if (res < 0) {
        return res;
    }
