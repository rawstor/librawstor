...
TEST(OstIOTest, set_object_fail) {
    ...
    // Slot::open()'s own first attempt (against the backend create()
    // already connected) plus invalidate_backend()'s own internal retry
    // (rawstor_opts_io_attempts() attempts) -- one more than io_attempts
    // total.
...
TEST(OstIOTest, write_disconnect_concurrent) {
    ...
    // between, so Backend::_add_op() runs for both before the client's
    ...
    // into: registered in Backend::_ops, but not yet "in flight" by
    // BackendOp::in_flight()'s old (now removed) definition.
    for (unsigned int i = 0; i < rawstor_opts_io_attempts(); ++i) {
        rawstor::tests::Session s(server);
        s.cmd_set_object(RAWSTOR_MAGIC, 0, 0);
    }
