...
void object_write_single(
    ...
    ASSERT_EQ(rawstor_object_open(queue, target.c_str(), &arm), 0);
...
TEST(MirrorResyncTest, probe_rejoins_recreated_arm) {
    ...
    ASSERT_EQ(rawstor_object_create(arms.target(2).c_str(), &spec), 0);
    ...
    RawstorObjectMeta c{};
    ASSERT_EQ(rawstor_object_meta(arms.target(0).c_str(), &a), 0);
    ASSERT_EQ(rawstor_object_meta(arms.target(2).c_str(), &c), 0);
    EXPECT_EQ(a.sync_id, c.sync_id);
    EXPECT_EQ(c.state, RAWSTOR_OBJECT_STATE_CLEAN);

    RawstorObject* arm = nullptr;
    ASSERT_EQ(rawstor_object_open(queue, arms.target(2).c_str(), &arm), 0);
    std::string data(4, '\0');
    object_read(queue, arm, data.data(), data.size(), 0);
    EXPECT_EQ(data, "ping");
    EXPECT_EQ(rawstor_object_close(arm), 0);
