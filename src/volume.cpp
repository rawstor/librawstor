...
void Volume::_rw_segments(
    std::shared_ptr<std::vector<VolumeSegment>> segments, bool write, bool sync,
    ...
    for (const VolumeSegment& segment : *segments) {
        ...
        _with_chunk(
            segment.index, [segment, write, sync, buf,
                            done](Object* object, int error) mutable {
                if (error) {
                    done(0, error);
                    return;
                }
...
void Volume::pread(
    ...
    _rw_segments(segments, false, /*sync=*/false, buf, std::move(cb));
}
...
void Volume::pwrite(
    ...
    _rw_segments(segments, true, sync, const_cast<void*>(buf), std::move(cb));
}
...
void Volume::preadv(
    ...
    _rw_segments(
        segments, false, /*sync=*/false, bounce->data(),
...
void Volume::pwritev(
        ...
        _with_chunk(
            ...
            [iov, niov, size, segment, sync,
             cb = std::move(cb)](Object* object, int error) mutable {
                ...
                object->pwritev(
                    iov, niov, size, segment.chunk_offset, sync, std::move(cb)
                );
            }
        );
        return;
    ...
    _rw_segments(
        segments, true, sync, bounce->data(),
...
void Volume::flush(std::function<void(int)>&& cb) {
    struct Fan {
        size_t pending = 0;
        int error = 0;
        std::function<void(int)> cb;
    ...
    for (Chunk& chunk : _chunks) {
        ...
        chunk.object->flush([fan](int error) {
            if (error && fan->error == 0) {
                fan->error = error;
            }
            if (--fan->pending == 0) {
