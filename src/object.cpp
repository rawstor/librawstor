...
struct FanoutState {
    ...
    std::vector<rawstd::URI> targets;
    std::function<
        void(rawio::Queue&, const rawstd::URI&, std::function<void(int)>&&)>
        op;
    std::function<void(int)> cb;
    size_t next;
    int error;
...
Object::Object(rawio::Queue& queue, const std::vector<rawstd::URI>& targets) :
    ...
    int res = parse_object_ref(targets.front().path().filename(), &_id, &_snap);
    if (res < 0) {
        RAWSTD_THROW_SYSTEM_ERROR(-res);
    }
...
void Object::snapshot(
    ...
    /* A snapshot binding is immutable — nothing new to snapshot. */
    if (_snap != 0) {
        cb({}, EROFS);
        return;
    }

    flush([this, snap_id, cb = std::move(cb)](int flush_error) mutable {
        if (flush_error) {
            cb({}, flush_error);
            return;
        }
        ...
        auto op = std::make_shared<FanOut>(
            FanOut{idxs.size(), 0, idxs, std::move(cb)}
        );

        for (size_t idx : idxs) {
            _members[idx].cn->snapshot(
                _id, snap_id,
                [op, alive = std::weak_ptr<int>(_alive)](int error) {
                    if (alive.expired()) {
                        return;
                    }
                    if (error) {
                        rawstd_error(
                            "Mirror member snapshot failed: %s\n",
                            strerror(error)
                        );
                        if (op->error == 0) {
                            op->error = error;
...
void Object::snap_remove(uint64_t snap_id, std::function<void(int)>&& cb) {
    std::vector<size_t> idxs;
    for (size_t i = 0; i < _members.size(); ++i) {
        if (_members[i].reachable) {
            idxs.push_back(i);
        }
    }
    ...
    auto op = std::make_shared<FanOut>(FanOut{idxs.size(), 0, std::move(cb)});

    for (size_t idx : idxs) {
        _members[idx].cn->snap_remove(
            _id, snap_id, [op, alive = std::weak_ptr<int>(_alive)](int error) {
                if (alive.expired()) {
                    return;
                }
                if (error) {
                    rawstd_error(
                        "Mirror member snapshot removal failed: %s\n",
                        strerror(error)
                    );
                    if (op->error == 0) {
                        op->error = error;
        ...
        [this, c, src, len, off, alive, buf,
         generation](size_t result, int error) {
            ...
            _members[_resync->idx].cn->pwrite(
                buf->data(), len, (off_t)off, /*sync=*/false,
...
void Object::snapshot(
    ...
    std::shared_ptr<FanoutState> st = std::make_shared<FanoutState>(FanoutState{
        queue, targets,
        [snap_id](
            rawio::Queue& q, const rawstd::URI& target,
            std::function<void(int)>&& done
        ) {
            rawstor::Connection::snapshot(q, target, snap_id, std::move(done));
        },
        std::move(cb), 0, 0
    });
    fanout_next(st);
...
void Object::_read_repair(size_t idx, off_t offset, std::vector<char>&& data) {
    ...
    _with_dirty([this, idx, offset, payload, alive](int error) {
        ...
        _members[idx].cn->pwrite(
            payload->data(), payload->size(), offset, /*sync=*/false,
...
void Object::pwrite(
    ...
    _with_dirty([this, buf, size, offset, sync, trace_event,
        ...
        _fan_out_write(
            offset, size,
            [buf, size, offset,
             sync](Connection& cn, std::function<void(size_t, int)>&& c) {
                cn.pwrite(buf, size, offset, sync, std::move(c));
            },
            [trace_event, cb = std::move(cb)](size_t result, int error) {
                RAWSTD_TRACE_EVENT_MESSAGE(
                    trace_event, "result = %zu, error = %d\n", result, error
...
void Object::pwritev(
    ...
    _with_dirty([this, iov, niov, size, offset, sync, trace_event,
                 cb = std::move(cb)](int error) mutable {
        if (error) {
            cb(0, error);
            return;
        ...
        _fan_out_write(
            ...
            [iov, niov, size, offset,
             sync](Connection& cn, std::function<void(size_t, int)>&& c) {
                cn.pwritev(iov, niov, size, offset, sync, std::move(c));
            },
            [trace_event, cb = std::move(cb)](size_t result, int error) {
                RAWSTD_TRACE_EVENT_MESSAGE(
                    trace_event, "result = %zu, error = %d\n", result, error
...
void Object::flush(std::function<void(int)>&& cb) {
    ...
    _fan_out_write(
        ...
        [](Connection& cn, std::function<void(size_t, int)>&& c) {
            cn.flush([c = std::move(c)](int error) { c(0, error); });
        },
...
void Object::close(std::function<void(int)>&& cb) {
    ...
    _settle_meta([this, cb = std::move(cb)]() mutable {
        ...
        flush([this, cb = std::move(cb)](int flush_error) mutable {
            if (_in_sync_count() == 0) {
...
int rawstor_volume_snapshot(const char* target, uint64_t* snap_id) noexcept {
    try {
        std::vector<rawstd::URI> uris = rawstd::URI::uriv(target);
        if (!is_volume_target(uris)) {
            rawstd_error("A single mds:// volume target expected\n");
            return -EINVAL;
        }
        volume_sync(
            [&uris, snap_id](rawio::Queue& q, std::function<void(int)>&& done) {
                rawstor::Volume::snapshot(q, uris[0], snap_id, std::move(done));
            }
        );
        return 0;
    } catch (const std::system_error& e) {
...
int rawstor_volume_snapshot_async(
    ...
    try {
        ...
        rawstor::Volume::snapshot(
            *static_cast<rawio::Queue*>(queue), uris[0], snap_id,
            [cb, data](int error) { cb(error ? -error : 0, data); }
        );
        return 0;
    } catch (const std::system_error& e) {
...
int rawstor_volume_snap_remove(const char* target, uint64_t snap_id) noexcept {
    try {
        std::vector<rawstd::URI> uris = rawstd::URI::uriv(target);
        if (!is_volume_target(uris)) {
            rawstd_error("A single mds:// volume target expected\n");
        ...
        volume_sync([&uris, snap_id](
                        rawio::Queue& q, std::function<void(int)>&& done
                    ) {
            rawstor::Volume::snap_remove(q, uris[0], snap_id, std::move(done));
        });
        return 0;
    } catch (const std::system_error& e) {
        return -e.code().value();
